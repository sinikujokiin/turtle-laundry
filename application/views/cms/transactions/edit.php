				...
				<div class="box-body">
					<div class="row">
						<?php $error = $this->session->flashdata('error') ?>
						    <form class="settings-form" method="post" action="<?= base_url('transactions/update/'.encrypt_decrypt('encrypt',$transactions->id)) ?>">
						    	<div class="mb-3">
								    ...
								    <select class="form-control" name="member_id" required id="member_id">
								    	...
								    	<?php foreach ($members as $value): ?>
								    		<option value="<?= $value->id ?>" <?= set_value('member_id', $transactions->member_id) == $value->id ? 'selected' : '' ?>><?= $value->name ?> </option>
								    	<?php endforeach ?>
								    </select>
								...
								<div class="row">
									...
									<div class="col-6">
										<div class="mb-3">
											...
											<input type="time" min="" class="form-control" name="due_time" value="<?= set_value("due_time",date("H:i:s")) ?>" required id="due_time">
								...
								<div class="bg-light p-3 b-2 border-primary">
									...
									<div id="multiple-service">
						                <?php foreach ($transactions->transactiondetail as $key => $value): ?>
											<div class="row">
												<div class="col-lg-4">
												    <div class="mb-3">
													    	...
													    	<?php foreach ($services as $group): ?>
														    	<optgroup label="<?= $group->name ?>">
														    		<?php foreach ($group->detail as $opt): ?>
														    			<option value="<?= $opt->id ?>" <?= set_value('service_detail_id', $value->service_detail_id) == $opt->id ? 'selected' : '' ?> data-price="<?= $opt->price ?>" data-unit="<?= $opt->unit ?>"><?= $opt->name ?> [<?= $opt->price.'/'.$opt->unit ?>]</option>
														    		<?php endforeach ?>
														    	</optgroup>
												...
												<div class="col-lg-3">
													<div class="mb-2">
														...
														<div class="input-group">
														    <input type="hidden" class="form-control" id="price_0" readonly value="<?= set_value('price', $value->price) ?>" name="price[]" required placeholder="10000">
														    <input type="text" class="form-control text-end" id="price_show_0" readonly value="<?= set_value('price', 'Rp. '.number_format($value->price,2,',','.')) ?>" name="price_show" required placeholder="10000">
															<span class="input-group-text" id="unit-text-0"><?= $value->servicedetail->unit ?></span>
														</div>
												...
												<div class="col-lg-2">
													<div class="mb-3">
														...
														<div class="input-group">
															<input type="number" step="0.5" min="1" class="form-control weight" name="weight[]" value="<?= set_value("weight",$value->weight) ?>" required id="weight_0">
															<span class="input-group-text" id="text-unit-0"><?= $value->servicedetail->unit ?></span>
														</div>
												...
												<div class="col-lg-2">
												    <div class="mb-3">
													    ...
													    <input type="hidden" readonly class="form-control subtotal" id="subtotal_0" value="<?= set_value('subtotal'),$value->price*$value->weight ?>" name="subtotal[]" required placeholder="0">
													    <input type="text" readonly class="form-control text-end" id="subtotal_show_0" value="<?= set_value('subtotal'),'Rp. '.number_format($value->price*$value->weight,2,',','.') ?>" name="subtotal_show" required placeholder="0">
									...
									<div class="row">
										...
										<div class="col-lg-3">
											<div class="mb-3">
							    			    ...
							    			    <div class="input-group">
							    			    	<input type="number" class="form-control text-end" name="discount" id="discount" value="<?= set_value('discount',$transactions->discount) ?>">
										...
										<div class="col-lg-3">
											<div class="mb-3">
							    			    ...
							    			    <div class="input-group">
							    			    	<input type="number" class="form-control text-end" name="tax" id="tax" value="<?= set_value('tax',$transactions->tax) ?>">
							    			    	<span class="input-group-text">%</span>
							    			    </div>
										...
										<div class="col-lg-3">
							    		    <div class="mb-3">
							    			    ...
							    			    <input type="hidden" readonly class="form-control" id="total" value="<?= set_value('total', $transactions->total) ?>" name="total" required placeholder="0">
							    			    <input type="text" readonly class="form-control text-end" id="total_show" value="<?= set_value('total', 'Rp. '.number_format($transactions->total,2,',','.')) ?>" name="total_show" required placeholder="0">
							    ...
							    <div class="mb-3">
									...
									<div class="form-check">
										<input class="form-check-input" type="radio" <?= set_value('payment_status', $transactions->payment_status) == 'sudah dibayar' ? 'checked' : '' ?> value="sudah dibayar" id="payment_status-1" name="payment_status" checked>
									 	<label class="form-check-label" for="payment_status-1">
									    	Sudah Dibayar
										</label>
										<input class="form-check-input" type="radio" <?= set_value('payment_status', $transactions->payment_status) == 'belum dibayar' ? 'checked' : '' ?> value="belum dibayar" id="payment_status-0" name="payment_status" >
